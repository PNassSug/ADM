...
Public Class empresaobrigacoesrelarioXtraForm
    Private bsEmpresaObrigacacoes As BindingSource
    ...
    Public Sub New()
        ...
        objComum.Browse("select codigo as tipolucro, descricao from tabaux where tabela = 'TipoLucro' order by codigo", tipolucroBindingSource)
        objComum.Browse("select codigo as tipoempresa, descricao from tabaux where tabela = 'TipoEmpresaEscrita' order by codigo", tipoempresaBindingSource)
    End Sub
    ...
    Private Sub imprimirSimpleButton_Click(sender As System.Object, e As System.EventArgs) Handles imprimirSimpleButton.Click
        Try
            If (tiporelatorioComboBoxEdit.SelectedIndex = 0) Then
                Dim report As New empresaobrigacoesEOXtraReport()
                GeraRelatorio(report)
            Else
                Dim report As New empresaobrigacoesOEXtraReport()
                GeraRelatorio(report)
            End If
    ...
    Private Sub GeraRelatorio(pReport As XtraReport)
        Try
            SplashScreenManager.ShowForm(Me, GetType(aguardeWaitForm), True, True, False)
            objEmpresaObrigacoes.Report(empresainicialSearchLookUpEdit.Text, empresafinalSearchLookUpEdit.Text, obrigacaoinicialSearchLookUpEdit.Text, obrigacaofinalSearchLookUpEdit.Text, tipolucroSearchLookUpEdit.Text, tipoempresaSearchLookUpEdit.Text, pReport)

            Dim printTool As New ReportPrintTool(pReport)
            pReport.Parameters.Item("Sistema").Value = Application.ProductName
            pReport.Parameters.Item("Sistema").Visible = False
    ...
    Private Sub tipolucroSearchLookUpEdit_CustomDisplayText(sender As System.Object, e As DevExpress.XtraEditors.Controls.CustomDisplayTextEventArgs) Handles tipolucroSearchLookUpEdit.CustomDisplayText
        Dim editor As SearchLookUpEdit = TryCast(sender, SearchLookUpEdit)
        If editor.EditValue IsNot Nothing Then
            Dim index As Integer = editor.Properties.GetIndexByKeyValue(editor.EditValue)
            If index >= 0 Then
                tipolucrodescTextEdit.Text = objComum.RetornaDescricao(tipolucroBindingSource, index, "descricao")
            Else
                tipolucrodescTextEdit.Text = String.Empty
            End If
        Else
            tipolucrodescTextEdit.Text = String.Empty
        End If
    ...
    Private Sub tipoempresaSearchLookUpEdit_CustomDisplayText(sender As System.Object, e As DevExpress.XtraEditors.Controls.CustomDisplayTextEventArgs) Handles tipoempresaSearchLookUpEdit.CustomDisplayText
        Dim editor As SearchLookUpEdit = TryCast(sender, SearchLookUpEdit)
        If editor.EditValue IsNot Nothing Then
            Dim index As Integer = editor.Properties.GetIndexByKeyValue(editor.EditValue)
            If index >= 0 Then
                tipoempresadescTextEdit.Text = objComum.RetornaDescricao(tipoempresaBindingSource, index, "descricao")
            Else
                tipoempresadescTextEdit.Text = String.Empty
            End If
        Else
            tipoempresadescTextEdit.Text = String.Empty
        End If
